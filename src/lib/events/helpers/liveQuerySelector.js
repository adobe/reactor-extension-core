...
function injectLiveQuerySelector({ WeakMap }) {
  ...
  const calledCallbacksByElement = new WeakMap();

  // Create a naked object with no prototype so we can safely use it as a map.
  let callbacksBySelector = Object.create(null);
  ...
  const findElements = function () {
    ...
    const selectors = Object.keys(callbacksBySelector);
    for (let i = 0; i < selectors.length; i++) {
      const selector = selectors[i];
      const callbacks = callbacksBySelector[selector];
  ...
  let initializePolling = once(function () {
   ...
   */
  function liveQuerySelector(selector, callback) {
    let callbacks = callbacksBySelector[selector];
    ...
    callbacks.push(function (element) {
      let calledCallbacks = calledCallbacksByElement.get(element);
  ...
  return { liveQuerySelector, __reset };
}

const validateInjection = validateInjectedParams(injectLiveQuerySelector);

const { liveQuerySelector, __reset } = validateInjection({
  WeakMap
});
...
/* START.TESTS_ONLY */
export { validateInjection as injectLiveQuerySelector };
/* END.TESTS_ONLY */
