...
function injectCustomEvent({ window }) {
  const bubbly = createBubbly();
  // todo look around the PR and make sure arrays like this are inside the injector
  const typesWatched = [];
  /**
   ...
   */
  return function customEvent(settings, trigger) {
...
const validateInjection = validateInjectedParams(injectCustomEvent);
...
/* START.TESTS_ONLY */
export { validateInjection as injectCustomEvent };
/* END.TESTS_ONLY */
