...
function injectKeyUp({ document }) {
   ...
   */
  return function keyUp(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectKeyUp);
...
/* START.TESTS_ONLY */
export { validateInjection as injectKeyUp };
/* END.TESTS_ONLY */
