...
function injectBlur({ document }) {
   ...
   */
  return function blur(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectBlur);
...
/* START.TESTS_ONLY */
export { validateInjection as injectBlur };
/* END.TESTS_ONLY */
