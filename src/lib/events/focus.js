...
function injectFocus({ document }) {
   ...
   */
  return function focusEvent(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectFocus);
...
/* START.TESTS_ONLY */
export { validateInjection as injectFocus };
/* END.TESTS_ONLY */
