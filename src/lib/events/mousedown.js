...
function injectMouseDown({ document }) {
   ...
   */
  return function mouseDown(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectMouseDown);
...
/* START.TESTS_ONLY */
export { validateInjection as injectMouseDown };
/* END.TESTS_ONLY */
