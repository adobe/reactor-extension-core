...
function injectMediaStalled({ document }) {
   ...
   */
  return function mediaStalled(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectMediaStalled);
...
/* START.TESTS_ONLY */
export { validateInjection as injectMediaStalled };
/* END.TESTS_ONLY */
