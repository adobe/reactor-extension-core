...
function injectMediaPause({ document }) {
   ...
   */
  return function mediaPauseEvent(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectMediaPause);
...
/* START.TESTS_ONLY */
export { validateInjection as injectMediaPause };
/* END.TESTS_ONLY */
