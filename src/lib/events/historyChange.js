...
function injectHistoryChange({ window, debounce }) {
   ...
   */
  const callThrough = function (object, methodName, fn) {
   ...
   */
  const callTriggersIfURIChanged = debounce(function () {
   ...
   */
  const watchForHistoryChange = once(function () {
    if (history) {
   ...
   */
  return function historyChange(settings, trigger) {
    watchForHistoryChange();
    triggers.push(trigger);
  };
}

const validateInjection = validateInjectedParams(injectHistoryChange);
...
/* START.TESTS_ONLY */
export { validateInjection as injectHistoryChange };
/* END.TESTS_ONLY */
