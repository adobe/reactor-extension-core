...
function injectSubmit({ document }) {
   ...
   */
  return function submitEvent(settings, trigger) {
    bubbly.addListener(settings, trigger);
  };
}

const validateInjection = validateInjectedParams(injectSubmit);
...
/* START.TESTS_ONLY */
export { validateInjection as injectSubmit };
/* END.TESTS_ONLY */
