  ...
  describe('lists of varying size', function () {
    describe('as strings', function () {
      describe('returns false when', function () {
        it('the list is of size 1', function () {
          var settings = { page: [{ value: 'http://foo.com/bar.html' }] };
          expect(conditionDelegate(settings)).toBe(false);
        });

        it('the list has many items', function () {
          var settings = {
            page: [{ value: 'bizzy' }, { value: 'bazzy' }, { value: 'buzzy' }]
          };
          expect(conditionDelegate(settings)).toBe(false);
        });
      });
      ...
      describe('returns true when', function () {
        it('the list is of size 1', function () {
          var settings = {
            page: [{ value: 'http://landingpage.com/test.html' }]
          };
          expect(conditionDelegate(settings)).toBe(true);
        });
    ...
    describe('as RegularExpressions', function () {
      describe('returns false when', function () {
        it('the list is of size 1', function () {
          var settings = {
            page: [{ value: 'g.o', pageIsRegex: true }]
          };
          expect(conditionDelegate(settings)).toBe(false);
        });
      ...
      describe('returns true when', function () {
        it('the list is of size 1', function () {
          var settings = {
            page: [{ value: 'Landingpage\\.com\\/t.st', pageIsRegex: true }]
          };
          expect(conditionDelegate(settings)).toBe(true);
        });
