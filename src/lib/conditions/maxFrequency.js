...
function injectMaxFrequency({ visitorTracking, getNamespacedStorage }) {
   ...
   * @returns {boolean}
   */
  return function maxFrequency(settings, event) {
...
const validateInjection = validateInjectedParams(injectMaxFrequency);
...
/* START.TESTS_ONLY */
export { validateInjection as injectMaxFrequency };
/* END.TESTS_ONLY */
